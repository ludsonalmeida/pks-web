...
export default function Pronto() {
  ...
  React.useEffect(() => { try { setBonus(window.sessionStorage.getItem('porks:reserva:bonus') || ''); } catch { /* ok */ } }, []);

  React.useEffect(() => {
    if (!code) return;
    apiGet<Reservation>(`/v1/reservations/public/lookup?code=${encodeURIComponent(String(code).toUpperCase())}`)
  ...
  const unitName = (r.unitRef?.name || r.unit || 'Porks Sobradinho').replace(/,.*$/, '');
  ...
  return (
    <>
      ...
      <div className={s.ticket}>
        ...
        <div className={s.ticketMeta}>{r.people} pessoas · {area}{r.reservationType === 'ANIVERSARIO' ? ' · Aniversário' : ''}</div>
        {bonus && <div className={`${s.ticketMeta} ${s.okText}`}>{bonus} garantido na mesa pelo aniversário informado.</div>}
      ...
      <div className={s.actions}>
        ...
        <p className={s.footerHint}>Convidado com código na mão chega mais. E de quinta a domingo, a partir de 5 na mesa, os chopes vêm por conta da casa.</p>
        <div className={s.secondary}>
          ...
          <a className={s.ghost} href={conciergeLink(meta?.atendimento || '61935003917', `Oi! Preciso alterar ou cancelar minha reserva ${r.reservationCode} (${when.format('DD/MM')} às ${when.format('HH:mm')}).`)} target="_blank" rel="noreferrer" onClick={() => track('post_confirm_action', { action: 'change' })}><IconPencil size={18} stroke={2.2} /> Alterar ou cancelar</a>
        </div>
      </div>
    </>
  );
