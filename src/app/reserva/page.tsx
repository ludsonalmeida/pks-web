...
import { type UnitOption } from './_components/UnitPicker';
...
import { PromoLadder } from './_components/PromoLadder';
...
import {
  ...
  ruleCovers, type RecurringRule, fmtLongDate, peakMinPeople, unitRulesForDay, lastBookableSlot, promoFor, isPromoWindow, PROMO_TIERS,
} from './_lib/rules';
...
function Tela1() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setUnits(norm);
        // Porks tem uma casa só: entra selecionada, sem pergunta
        const want = params.get('unit');
        if (!draft.unitId) {
          const u = (want && norm.find((x) => x.id === want || x.slug === want)) || norm[0];
          if (u) selectUnit(u);
        }
        track('reserve_start', { source: params.get('utm_source') || 'direto' });
      } catch {
  ...
  const fullSlots = React.useMemo(() => {
    ...
    for (let h = 12; h <= 23; h++) for (const m of ['00', '30']) {
  ...
  const suggestedId = React.useMemo(() => {
    ...
    if (draft.kids > 0) {
      const fam = ok.find((a) => /coberta|fam[ií]lia/i.test(a.name));
  ...
  const promo = promoFor(draft.dateYMD, draft.time, total);
  const promoWindow = isPromoWindow(draft.dateYMD, draft.time);
  ...
  const answers: Record<QKey, string | null> = {
    unit: draft.unitId ? (meta?.short || draft.unitName || '') : null,
    people: peopleDone && draft.adults > 0 && !belowMin && !tooBig ? `${total} pessoas` : null,
  ...
  const order: QKey[] = ['people', 'date', 'time'];
  ...
  if (!ready) {
    return (
      <>
        <StepHeader step={1} />
        <div className={s.intro}><h1 className={s.h1}>Reserve sua mesa e garanta chope cortesia para seus convidados</h1></div>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <div className={s.intro}>
        <h1 className={s.h1}>Reserve sua mesa e garanta chope cortesia para seus convidados</h1>
      </div>

      {/* 1. pessoas (a casa é uma só, já entra escolhida) */}
      <Question id="people" title="Quem vem?" aside="o número exato ajuda a montar a mesa" index={1} state={stateOf('people')} answer={answers.people} onEdit={() => setEditing('people')}>
        <div className={s.counterCard}>
          <PeopleCounter id="reserva-pessoas" label="Pessoas" helper="Conte você também" value={draft.adults} min={0} max={200}
            quick={[2, 4, 5, 6, 8, 10, 12]} onChange={(n) => patch({ adults: n, kids: 0 })} />
        ...
        {draft.adults > 0 && !tooBig && <PromoLadder people={total} />}
        {tooBig ? (
          <div className={`${s.alert} ${s.alertInfo}`} role="status">
            ...
            <span>
              ...
              <a className={s.linkBtn} href={conciergeLink(meta?.concierge || '61981776251', `Oi! Quero reservar para ${total} pessoas no ${meta?.short || 'Porks'}. Podem me ajudar?`)} target="_blank" rel="noreferrer">Chamar a equipe</a>
            </span>
          </div>
        ) : (
      ...
      {/* 2. dia */}
      <Question id="date" title="Que dia?" index={2} state={stateOf('date')} answer={answers.date} onEdit={() => setEditing('date')}>
        ...
        <p className={s.hint}>Reservas das 18h às 21h, em qualquer dia aberto. Segunda a casa não abre.</p>
      </Question>

      {/* 3. horário */}
      <Question id="time" title="Que horas vocês chegam?" aside="a mesa espera 15 min" index={3} state={stateOf('time')} answer={answers.time} onEdit={() => setEditing('time')}>
      ...
      {showArea && (
        <>
          <div className={s.reveal}>
            <PromoLadder people={total} dateYMD={draft.dateYMD} time={draft.time} compact />
            {!promo && (promoWindow ? total < PROMO_TIERS[0].min : true) && (
          ...
          <section className={`${s.block} ${s.reveal}`} aria-labelledby="q-occ" data-step={4}>
            <h2 className={s.q} id="q-occ"><span className={s.qTitle}><span className={s.stepBadge} aria-hidden="true">4</span>É uma ocasião especial?</span><small>opcional</small></h2>
            ...
            <p className={s.hint}>Ajuda a equipe a preparar a mesa do jeito certo.</p>
          </section>

          <section className={`${s.block} ${s.reveal}`} aria-labelledby="q-area" data-step={5}>
            <h2 className={s.q} id="q-area"><span className={s.qTitle}><span className={s.stepBadge} aria-hidden="true">5</span>Onde vocês querem ficar?</span></h2>
            {areas.length === 0 && availLoading ? (
              ...
              <div className={`${s.alert} ${s.alertBad}`} role="status">
                ...
                <span>
                  ...
                  {' '}ou{' '}
                  <a className={s.linkBtn} href={conciergeLink(meta?.concierge || '61981776251', `Oi! Queria uma mesa para ${total} no ${meta?.short || 'Porks'} ${fmtLongDate(draft.dateYMD!)} às ${draft.time}, mas o site mostrou lotado. Tem alguma opção?`)} target="_blank" rel="noreferrer">falar com a gente no WhatsApp</a>.
                </span>
              </div>
            ) : (
              <>
                ...
                <p className={s.hint}>
                  {draft.areaId === suggestedId
                    ? draft.kids > 0 ? 'Sugerimos a Coberta porque vêm crianças: fica no salão, perto de tudo. Toque em outra para trocar.' : 'Sugerimos a área com mais lugar para o grupo. Toque em outra para trocar.'
