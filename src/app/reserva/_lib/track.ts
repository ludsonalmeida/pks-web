...
export function track(event: FunnelEvent, props: Record<string, unknown> = {}) {
  ...
  try {
    ...
    // O Agente do Porks escuta os eventos do funil pra oferecer ajuda no momento certo (lotou, erro, travou).
    w.dispatchEvent(new CustomEvent('porks:funil', { detail: payload }));
