'use client';
// Agente do Porks: botão flutuante + painel de chat dentro da jornada de reserva.
// PASSO 1 (demonstração): respostas locais, sem IA. Nos passos 2 e 3 o cérebro passa a ser um agente de IA via proxy no servidor.
...
function demoReply(text: string, d: Draft, nextId: () => number): Msg[] {
  ...
  if (/crian|kids|brinq|fam[ií]lia/.test(q)) {
    return [{ id: nextId(), who: 'bot', text: `Família é bem-vinda no Porks. Com criança, a área Coberta é a mais prática: fica no salão, perto da banda e do banheiro. O Deck é ao ar livre, bom de tarde. ${d.kids ? `Vi que vocês vêm com ${d.kids} criança${d.kids > 1 ? 's' : ''}, então a Coberta já entra como sugestão.` : 'Se quiser, marco a Coberta pra você.'}` }];
  }
  ...
  if (/cancel|alter|mudar|troc/.test(q)) {
    return [{ id: nextId(), who: 'bot', text: 'Depois de confirmar você recebe um código. Pra alterar dia, hora ou quantidade, ou cancelar, é pelo WhatsApp oficial do Porks, (61) 93500-3917. Sem multa, sem burocracia.' }];
  }
  if (/estacion|carro|chegar|onde|endere/.test(q)) {
    return [{ id: nextId(), who: 'bot', text: `O ${casa} fica em Sobradinho, DF. Chegando, é só mostrar o código da reserva na entrada que a mesa já está no seu nome.` }];
  }
  if (/card[aá]pio|comer|prato|chope|bebida|pre[çc]o/.test(q)) {
    return [{ id: nextId(), who: 'bot', text: 'Porco e chope, sem couvert e sem 10%. Costelinha, torresmo mineiro, pururuca, pernil desfiado, barriga crocante, chope Pilsen, IPA e Stout. O cardápio completo está em sobradinhoporks.com.br.' }];
  }
...
export default function AgenteMane() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('porks:funil', onFunil);
    ...
    return () => {
      window.removeEventListener('porks:funil', onFunil);
  ...
  const abrir = (origem: string) => {
    ...
    if (msgs.length === 0) {
      ...
      const oi = r
        ? `Oi! Sou o Agente do Porks, uma inteligência artificial. Vi que você está montando uma reserva: ${r}. Em que posso ajudar?`
        : 'Oi! Sou o Agente do Porks, uma inteligência artificial. Posso ajudar a escolher horário e ambiente, e a fechar sua reserva. O que você precisa?';
      setTyping(true);
  ...
  const humano = useMemo(() => {
    const r = resumo(draft);
    return conciergeLink(meta?.concierge || '61981776251', `Oi! Estava fazendo uma reserva pelo site${r ? ` (${r})` : ''} e preciso de ajuda de uma pessoa.`);
  }, [draft, meta]);

  const quick = ['Outro horário', 'Como ganho os chopes?', 'Quanto tempo segura a mesa?', 'Como altero depois?'];
    ...
    <>
      ...
      {open && (
        <>
          ...
          <section className={s.panel} role="dialog" aria-modal="true" aria-label="Agente do Porks">
            <header className={s.head}>
              <span className={s.avatar} aria-hidden="true"><Spark /><span className={s.ia}>IA</span></span>
              <div className={s.headText}>
                <strong>Agente do Porks</strong>
